 ...
 */
/* eslint max-len: "off" */
const Widgets = ({ widgets, widgetTypes, onRemove, layout, columnIndex, rowIndex, editable, frameComponent, onMove, entryDragOver, entryDrop }) => {
  let createdWidgets = widgets.map((widget, index) => { // eslint-disable-line arrow-body-style
    //console.log("widget",widget);
    widget={...widget,
      onDrop:(event)=>entryDrop(event,widget.entyID),
      onDragOver:(event)=>entryDragOver(event,widget.entyID)}
    return (
      <WidgetFrame
        key={index}
      ...
      >
        {
          createElement(widgetTypes[widget.key].type, widget, widgetTypes[widget.key].props)
        }
      </WidgetFrame>
    );
  });
